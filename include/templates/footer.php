<!-- Footer -->

<section class="footer text-light">
   <div class="container">
       <div class="row" data-aos="fade-right">
           <div class="col-lg-3 py-4 py-md-5">
               <div class="d-flex align-items-center">
                   <h4 class="">جامعة الأوائل الحديثة</h4>
               </div>
               <p class="py-3 para-light">جامعة متميزة عالميا باستدامة وشراكة مجتمعية</p>
               <div class="d-flex">
                   <div class="me-3">
                       <a  target="_blank" href="https://www.facebook.com/AMU2022/">
                           <i class="fab fa-facebook-f fa-2x py-2"></i>
                       </a>
                   </div>
                   <div class="me-3">
                       <a target="_blank" href="https://twitter.com/alawaelmodern">
                           <i class="fab fa-twitter fa-2x py-2"></i>
                       </a>
                   </div>
                   <div class="me-3">
                       <a target="_blank" href="https://www.instagram.com/alawaelmodern/">
                           <i class="fab fa-instagram fa-2x py-2"></i>
                       </a>
                   </div>
               </div>
           </div> <!-- end of col -->

           <div class="col-lg-3 py-4 py-md-5">
               <div>
                   <h4 class="py-2">روابط سريعة</h4>
                   <div class="d-flex align-items-center py-2">
                       <i class="fas fa-caret-right"></i>
                       <a href="index.php#home"><p class="ms-3">عنا</p></a>
                   </div>
                   <div class="d-flex align-items-center py-2">
                       <i class="fas fa-caret-right"></i>
                       <a href="index.php#AdmissionSystem"><p class="ms-3">نظام القبول</p></a>
                   </div>
                   <div class="d-flex align-items-center py-2">
                       <i class="fas fa-caret-right"></i>
                       <a href="index.php#colleges"><p class="ms-3">الكليات</p></a>
                   </div>
                   <div class="d-flex align-items-center py-2">
                       <i class="fas fa-caret-right"></i>
                       <a href="index.php#grades"><p class="ms-3">الدرجات</p></a>
                   </div>
               </div>
           </div> <!-- end of col -->

           <div class="col-lg-3 py-4 py-md-5">
               <div>
                   <h4 class="py-2">روابط تهمك</h4>
                   <div class="d-flex align-items-center py-2">
                       <i class="fas fa-caret-right"></i>
                       <a href="#home"><p class="ms-3">سياسة الخصوصية</p></a>

                   </div>
                   <div class="d-flex align-items-center py-2">
                       <i class="fas fa-caret-right"></i>
                       <a href="#home"><p class="ms-3">الشروط والأحكام</p></a>
                   </div>
                   <!--<div class="d-flex align-items-center py-2">
                       <i class="fas fa-caret-right"></i>
                       <a href="#home"><p class="ms-3">Disclaimer</p></a>
                   </div>-->
                   <div class="d-flex align-items-center py-2">
                       <i class="fas fa-caret-right"></i>
                       <a href="#home"><p class="ms-3">اسئلة متكررة</p></a>
                   </div>
               </div>
           </div> <!-- end of col -->

           <div class="col-lg-3 py-4 py-md-5">
               <div class="d-flex align-items-center">
                   <h4>الاشتراك في الاخبار</h4>
               </div>
               <p class="py-3 para-light"></p>
               <div class="d-flex align-items-center">
                   <div class="input-group mb-3">
                       <input type="text" class="form-control p-2" placeholder="Enter Email" aria-label="Recipient's email">
                       <button class="btn-secondary text-light"><i class="fas fa-envelope fa-lg"></i></button>
                   </div>
               </div>
           </div> <!-- end of col -->
       </div> <!-- end of row -->
   </div> <!-- end of container -->
</section> <!-- end of footer -->
<!-- Bottom -->
<div class="bottom py-2 text-light" >
   <div class="container d-flex justify-content-between">
       <div>
           <p>حقوق © الطبع محفوظة</p>
           <p>لجامعة الأوائل الحديثة 2021</p>
       </div>
       <div>
           <i class="fab fa-cc-visa fa-lg p-1"></i>
           <i class="fab fa-cc-mastercard fa-lg p-1"></i>
           <i class="fab fa-cc-paypal fa-lg p-1"></i>
           <i class="fab fa-cc-amazon-pay fa-lg p-1"></i>
       </div>
   </div> <!-- end of container -->
</div> <!-- end of bottom -->


<!-- Back To Top Button -->
<button onclick="topFunction()" id="myBtn">
   <img src="./Layout/assets/images/up-arrow.png" alt="footer">
</button>
<!-- end of back to top button -->


<!-- Scripts -->
<script src="./Layout/js/bootstrap.min.js"></script><!-- Bootstrap framework -->
<script src="./Layout/js/purecounter.min.js"></script> <!-- Purecounter counter for statistics numbers -->
<script src="./Layout/js/swiper.min.js"></script><!-- Swiper for image and text sliders -->
<script src="./Layout/js/aos.js"></script><!-- AOS on Animation Scroll -->
<script src="./Layout/js/script.js"></script>  <!-- Custom scripts -->
<script src="./Layout/js/backend.js"></script> <!-- MY OWN Script -->
</body>
</html>
